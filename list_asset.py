...
@app.route('/list_asset/all', methods=['GET'])
def list_all_reserved():
    ...
    try:
        ...
        cursor.execute(
            "select asset_id,manufacturer,bmc_ip,bmc_user,asset_location,reserved,assigned_to,assigned_from,assigned_by,created_on,created_by,updated_on,updated_by,os_ip,os_user,purpose,cluster_id,delete,status from asset where reserved=true or reserved=false")
        res = cursor.fetchall()
        print(res)
